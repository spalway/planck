 ...
 * Mint a broker.
 *
 * Gated on holding $SBIT, which is the token's only job in the product. The
    ...
    <Section id="mint" label="06" title="mint a broker">
      ...
      <div className="flex flex-col gap-4">
        ...
        {holding.status === "unavailable" && holding.reason === "not_launched" && (
          <Panel>
            $SBIT has not launched yet, so there is nothing to hold and nothing to
        ...
        {holding.status === "known" && !holding.holding.holds && (
          <Panel>
            This wallet holds no $SBIT. Minting is open to holders; the contract
          ...
          <Panel>
            {result.reason === "mint_cap_reached"
              ...
              : result.reason === "not_holding"
                ? "That wallet no longer holds $SBIT."
